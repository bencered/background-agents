...
class SandboxSupervisor:
    ...
    def __init__(self):
        ...
        # Debug: log session config to verify MCP servers are passed through
        self.log.info("session_config.loaded",
                      keys=list(self.session_config.keys()),
                      has_mcp=("mcp_servers" in self.session_config),
                      mcp_count=len(self.session_config.get("mcp_servers") or []))
    ...
    def _setup_opencode_auth(self) -> None:
        ...
        try:
            ...
            self.log.info("opencode_auth.setup", providers=providers)
        except Exception as e:
    ...
    async def start_opencode(self) -> None:
        ...
        # Inject MCP servers if configured (from session config or control plane API)
        self.log.info("mcp.injection_start", session_mcp=self.session_config.get("mcp_servers"))
        mcp_servers = self.session_config.get("mcp_servers")
        if not mcp_servers:
            # Fallback: fetch MCP servers directly from control plane API
            try:
                ...
                if cp_url and cp_secret:
                    ...
                    if resp.status_code == 200:
                        ...
                        # Filter for global servers or servers scoped to this repo
                        ...
                        self.log.info("mcp.fetched_from_api", count=len(mcp_servers))
            except Exception as e:
                self.log.warn("mcp.api_fetch_failed", exc=str(e))
        ...
        # Write config to project .opencode directory as well (belt and suspenders)
        opencode_dir = workdir / ".opencode"
        opencode_dir.mkdir(parents=True, exist_ok=True)
        config_file = opencode_dir / "opencode.json"
        config_file.write_text(json.dumps(opencode_config, indent=2))
        self.log.info("opencode.config_written", path=str(config_file), keys=list(opencode_config.keys()))
