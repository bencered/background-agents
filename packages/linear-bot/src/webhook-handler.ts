...
async function handleNewSession(
  ...
  const issueDetails = await fetchIssueDetails(client, issue.id);
  const labels = issueDetails?.labels || (issue as any).labels || [];
  const labelNames = labels.map((l: { name: string }) => l.name);
  const projectInfo = issueDetails?.project || (issue as any).project;
  ...
  if (existingSession) {
    ...
    try {
      ...
      if (statusRes.ok) {
        ...
        isAlive = !terminalStatuses.has(sessionData.status as SessionStatus);
      }
    } catch {
